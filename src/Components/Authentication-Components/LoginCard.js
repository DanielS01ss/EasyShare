...
const LoginCard = () => {
  ...
  const [wrongPassOrUsErrMsg, setWrongPassOrUsErrMsg] = useState(false);
  ...
  const login = async () => {
    try {
      const reqResp = await loginReq(email, password);
      const token = reqResp.data.token;
      const refreshToken = reqResp.data.refreshToken;
    ...
    <div class="pt-40 pb-20 flex justify-center items-center h-screen ">
      ...
      <div class="w-96 bg-white shadow-md border border-gray-200 rounded-lg max-w-xl p-8 sm:p-6 lg:p-10 dark:bg-gray-800 dark:border-gray-700">
        <form class="space-y-6" action="#">
          ...
          <div>
            {wrongPassOrUsErrMsg && (
              <AuthErrorMsg text={"Incorrect username or password"} />
            )}
            <label
              for="email"
          ...
          <div>
            {formData.emailValid && formData.passwordNotEmpty ? (
              <button
                onClick={(e) => {
                  e.preventDefault();
                  login();
                  setWrongPassOrUsErrMsg(false);
                }}
